export const sidebarArticles = [
  {
    cover: {
      src:
        'http://www.gochikmagalur.com/siteImages/PRODUCTMASTER/1305/Coffee_Museum_1.jpg',
      srcSet:
        'http://www.gochikmagalur.com/siteImages/PRODUCTMASTER/1305/Coffee_Museum_1.jpg',
    },
    title: 'Two Famous Coffee Museums',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/kate-segelson.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/kate-segelson@2x.jpg 2x',
      },
      name: 'Kate Segelson',
    },
    date: '04 Aug',
    tags: ['Health', 'beans', 'drinks'],
  },
  {
    cover: {
      src:
        'https://www.roastycoffee.com/wp-content/uploads/Depositphotos_93910882_xl-2015-scaled.jpg',
      srcSet:
        'https://www.roastycoffee.com/wp-content/uploads/Depositphotos_93910882_xl-2015-scaled.jpg',
    },
    title: 'Coffee from Nicaragua',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/jack-smith.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/jack-smith@2x.jpg 2x',
      },
      name: 'Jack Smith',
    },
    date: '04 Aug',
    tags: ['coffee', 'break', 'intense'],
  },
  {
    cover: {
      src:
        'https://cdn.aarp.net/content/dam/aarp/health/drugs_supplements/2013-10/1140-cup-coffee-too-much-caffeine-bad.web.jpg',
      srcSet:
        'https://cdn.aarp.net/content/dam/aarp/health/drugs_supplements/2013-10/1140-cup-coffee-too-much-caffeine-bad.web.jpg',
    },
    title: 'Health benefits and risks of drinking coffee',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/akachi-luccini.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/akachi-luccini@2x.jpg 2x',
      },
      name: 'Akachi Luccini',
    },
    date: '04 Aug',
    tags: ['fresh', 'break', 'intense'],
  },
  {
    cover: {
      src:
        'https://www.maxespressocoffee.com/wp-content/uploads/2017/09/arabica-coffee-maxespresso.jpeg',
      srcSet:
        'https://www.maxespressocoffee.com/wp-content/uploads/2017/09/arabica-coffee-maxespresso.jpeg',
    },
    title: 'Interesting coffee facts',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/veronica-adams.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/veronica-adams@2x.jpg 2x',
      },
      name: 'Veronica Adams',
    },
    date: '04 Aug',
    tags: ['coffee', 'cups', 'morning coffee', 'breakfast'],
  },
];

export const similarStories = [
  {
    cover: {
      src:
        'https://www.roastycoffee.com/wp-content/uploads/Depositphotos_93910882_xl-2015-scaled.jpg',
      srcSet:
        'https://www.roastycoffee.com/wp-content/uploads/Depositphotos_93910882_xl-2015-scaled.jpg',
    },
    title: 'Coffee from Nicaragua‎',
    subtitle:
      'Adipisicing exercitation veniam ex laboris in commodo eu enim ea nulla in nulla non.',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/kate-segelson.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/kate-segelson@2x.jpg 2x',
      },
      name: 'Kate Segelson',
    },
    date: '04 Aug',
    tags: ['Health', 'beans', 'drinks'],
  },
  {
    cover: {
      src:
        'https://cdn.aarp.net/content/dam/aarp/health/drugs_supplements/2013-10/1140-cup-coffee-too-much-caffeine-bad.web.jpg',
      srcSet:
        'https://cdn.aarp.net/content/dam/aarp/health/drugs_supplements/2013-10/1140-cup-coffee-too-much-caffeine-bad.web.jpg',
    },
    title: 'Health benefits and risks of drinking coffee‎',
    subtitle: 'Laboris incididunt officia nisi amet aliqua voluptate fugiat.',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/jack-smith.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/jack-smith@2x.jpg 2x',
      },
      name: 'Jack Smith',
    },
    date: '04 Aug',
    tags: ['coffee', 'break', 'intense'],
  },
  {
    cover: {
      src:
        'https://www.maxespressocoffee.com/wp-content/uploads/2017/09/arabica-coffee-maxespresso.jpeg',
      srcSet:
        'https://www.maxespressocoffee.com/wp-content/uploads/2017/09/arabica-coffee-maxespresso.jpeg',
    },
    title: 'Interesting coffee facts',
    subtitle:
      'Magna enim culpa ex sint veniam non aliquip magna consectetur esse sit veniam ullamco.',
    author: {
      photo: {
        src:
          'https://assets.maccarianagency.com/the-front/photos/people/akachi-luccini.jpg',
        srcSet:
          'https://assets.maccarianagency.com/the-front/photos/people/akachi-luccini@2x.jpg 2x',
      },
      name: 'Akachi Luccini',
    },
    date: '04 Aug',
    tags: ['fresh', 'break', 'intense'],
  },
];
export const content = {
  headline:
    'We’ve come a long way from the cans of Folgers that filled our grandparents’ cupboards, with our oat milk lattes, cold brews and Frappuccinos. Some of us are still very utilitarian about the drink while others perform elaborate rituals. The fourth most popular beverage in the country, coffee is steeped into our culture. Just the right amount can improve our mood; too much may make us feel anxious and jittery.',
  quote: 'We don’t know for sure if coffee is the cause of the health benefits',
  text1: `In moderation, coffee seems to be good for most people — that’s 3 to 5 cups daily, or up to 400 milligrams of caffeine.

“The evidence is pretty consistent that coffee is associated with a lower risk of mortality,” said Erikka Loftfield, a research fellow at the National Cancer Institute who has studied the beverage.

For years, coffee was believed to be a possible carcinogen, but the 2015 Dietary Guidelines helped to change perception. For the first time, moderate coffee drinking was included as part of a healthy diet. When researchers controlled for lifestyle factors, like how many heavy coffee drinkers also smoked, the data tipped in coffee’s favor.`,
  text2: `A large 2017 review on coffee consumption and human health in the British Medical Journal also found that most of the time, coffee was associated with a benefit, rather than a harm. In examining more than 200 reviews of previous studies, the authors observed that moderate coffee drinkers had less cardiovascular disease, and premature death from all causes, including heart attacks and stroke, than those skipping the beverage.

In addition, experts say some of the strongest protective effects may be with Type 2 diabetes, Parkinson’s disease, and liver conditions such as cirrhosis, liver cancer and chronic liver disease. For example, having about five cups of coffee a day, instead of none, is correlated with a 30 percent decreased risk of Type 2 diabetes, according to a meta-analysis of 30 studies.

The potential benefit from coffee might be from the polyphenols, which are plant compounds that have antioxidant properties, according to Dr. Giuseppe Grosso, an assistant professor in human nutrition at University of Catania in Italy and the lead author of an umbrella review in the Annual Review of Nutrition.

However, coffee isn’t for everyone. There are concerns about overconsumption. This is especially true for expecting mothers because the safety of caffeine during pregnancy is unclear. While the research into coffee’s impact on health is ongoing, most of the work in this field is observational.`,
  title: 'Is Coffee Good for You?',
  subtitle: 'Yes! But it depends on the kind of coffee and the quantity.',
  author: {
    photo: {
      src:
        'https://assets.maccarianagency.com/the-front/photos/people/jack-smith.jpg',
      srcSet:
        'https://assets.maccarianagency.com/the-front/photos/people/jack-smith@2x.jpg 2x',
    },
    name: 'Jack Smith',
    date: 'May 20, 2019',
  },
  cover: {
    src:
      'https://brewabilitylab.com/wp-content/uploads/2019/07/caff%C3%A8-salute-pixabay.com_.jpg',
    srcSet:
      'https://brewabilitylab.com/wp-content/uploads/2019/07/caff%C3%A8-salute-pixabay.com_.jpg',
  },
  images: [
    {
      src:
        'https://i2.wp.com/www.healthfitnessrevolution.com/wp-content/uploads/2019/02/iStock-954537590.jpg',
      srcSet:
        'https://i2.wp.com/www.healthfitnessrevolution.com/wp-content/uploads/2019/02/iStock-954537590.jpg',
      cols: 2,
    },
    {
      src:
        'https://upload.wikimedia.org/wikipedia/commons/2/26/Drinking_Coffee_%28Unsplash%29.jpg',
      srcSet:
        'https://upload.wikimedia.org/wikipedia/commons/2/26/Drinking_Coffee_%28Unsplash%29.jpg',
      cols: 1,
    },
    {
      src:
        'https://assets.maccarianagency.com/the-front/photos/blog/cover1.jpg',
      srcSet:
        'https://assets.maccarianagency.com/the-front/photos/blog/cover1.jpg 2x',
      cols: 1,
    },
  ],
};
